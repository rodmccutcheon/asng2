...
def create_other_user
  @other_user = FactoryGirl.create(:user, :name => "Other Guy", :email => "other@example.com",
    :password => "otherguy", :password_confirmation => "otherguy")
...
When /^I change a users role$/ do
  create_other_user
  visit '/admin/users'
  within("tr#user_" + @other_user.id.to_s) do
...
Then /^I should see an user successfully updated message$/ do
...
Then /^I should see an user successfully deleted message$/ do
...
Then /^I should see all users listed$/ do
  users = User.all
  users.each do |user|
    page.should have_content user.name
  end
